...
from inference import simulate_forward, simulate_forward_FCFS
...
class SkipJoinMLFQScheduler(threading.Thread):
    ...
    def getNewRequest(self, request: Request):
        ...
        print(f"THREAD-SCHEDULER:putting request{request.j_id} (length={length}&ft={ft})into queue{index}")
...
class FCFS_Scheduler(threading.Thread):
    def __init__(self):
        super().__init__()
        self.request_queue = queue.Queue()  # 用于存储请求的队列
        self.executed = 0  # 已经完成的请求数量
        self.ave_jct = [] # 用于存储每个请求的JCT

    def getNewRequest(self, request):
        # 将新到达的请求按照先来先服务原则放入队列
    ...
    def getInferenceJob(self):
        ...
        if not self.request_queue.empty():
            ...
            print(f"FCFS-SCHEDULER: Getting Job {next_request.j_id} (length={next_request.prompt_length})")
            return next_request
        else:
            return None
    ...
    def run(self):
        while self.executed != JOB_NUM:
            ...
            print(f"FCFS-SCHEDULER: Submitting Job {job.j_id}")
            if job.iter_count == 0:
                iter_time = job.first_iter_time
            else:
                iter_time = job.next_iter_time
            ...
            args = [iter_time, job, self]
            # 调用模拟推理线程
            temp_thread = thread_pool.submit(lambda p: simulate_forward_FCFS(*p), args)
            temp_thread.result()